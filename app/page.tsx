...
import TestimonialVideos from '@/app/components/TestimonialVideos'
...
function CTA({ children, variant = "primary", style: s = {}, href, onClick }: CTAProps) {
...
function FeatureCard({ label, title, body, children, mobile }: FeatureCardProps) {
  return (
    <div style={{
      display: 'flex',
...
// Mockup Components
function MethodologyMockup({ mobile }: { mobile: boolean }) {
  const docs = [
...
function MemoryMockup({ mobile }: { mobile: boolean }) {
  const history = [
...
function ProgramMockup({ mobile }: { mobile: boolean }) {
  const exercises = [
...
function AttentionMockup({ mobile }: { mobile: boolean }) {
  const alerts = [
    { client: 'James P.', issue: 'Missed last 2 sessions', priority: 'high' as const },
    { client: 'Maria G.', issue: 'Check-in due today', priority: 'medium' as const },
    { client: 'Kyle D.', issue: 'Goal deadline in 5 days', priority: 'low' as const },
  ]
...
function StoryMockup({ mobile }: { mobile: boolean }) {
  return (
          ...
          {features.map((feature, idx) => (
            ...
            >
              ...
              >
                ...
                {idx === 0 && <MethodologyMockup mobile={mobile} />}
                {idx === 1 && <MemoryMockup mobile={mobile} />}
                {idx === 2 && <ProgramMockup mobile={mobile} />}
                {idx === 3 && <AttentionMockup mobile={mobile} />}
                {idx === 4 && <StoryMockup mobile={mobile} />}
              </FeatureCard>
            </section>
          ))}
        </div>
